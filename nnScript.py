...
def preprocess():
    ...
    # Feature selection
    # Your code here.
    b = np.all(train_data==train_data[0,:] ,0) #bool array returns T if each element of col is the same
    b = b==0 #negate boolean

    train_data=train_data[:,b]
    validation_data=validation_data[:,b]
    test_data = test_data[:,b]
    
    features={}
    features = np.array(np.where(b==1))
...
def nnObjFunction(params, *args):
    ...
    dl= o-y
    djdw2 = np.dot(np.transpose(dl),z)
    djdw1 = np.dot(np.transpose(np.dot(dl,w2)*(z*(1.0- z))),x)
    #djdw1 = (np.dot(np.dot(np.transpose(1-z[:,1:]),z[:,1:]), np.dot(np.transpose(np.dot(dl,w2[:,1:])),x))+n*lam*w1)/n
            
    
    J = -np.sum(np.sum(y*np.log(o)+(1-y)*np.log(1-o),1))/n
    Jreg = J + lam/2/n*(np.sum(w1**2)+np.sum(w2**2))
    
    djdw1reg = (djdw1[1:,:]+lam*w1)/n
    djdw2reg = (djdw2+lam*w2)/n
    ...
    obj_grad = np.concatenate((djdw1reg.flatten(), djdw2reg.flatten()),0)
    print(obj_val)
